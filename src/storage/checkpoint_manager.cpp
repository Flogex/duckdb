...
SingleFileCheckpointWriter::~SingleFileCheckpointWriter() {
	...
	// A destructor must not throw, and a read-only transaction leaves nothing to undo if the rollback fails.
	try {
...
void SingleFileCheckpointWriter::CreateCheckpoint() {
	...
	// Explicit checkpoints bind indexes (see WriteTable). Binding looks the table up in the catalog, so it needs a
	// transaction on this database. Start a read-only one here: a read-only start skips start_transaction_lock,
	// which FORCE CHECKPOINT holds while it waits for the checkpoint lock we already own.
...
void SingleFileCheckpointWriter::WriteTable(TableCatalogEntry &table, Serializer &serializer) {
	...
	if (bind_connection) {
		...
		// Read-only, so any transaction the binder still starts (e.g. on the system catalog) skips
		// start_transaction_lock.
