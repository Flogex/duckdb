...
private:
	...
	//! Read-only connection that binds indexes, only set while CreateCheckpoint runs. Its transaction never takes
	//! start_transaction_lock, which FORCE CHECKPOINT holds while it waits for the checkpoint lock we already own.
